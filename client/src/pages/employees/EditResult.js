...
function EditResult() {
  ...
  const getResult = async (values) => {
    try {
      ...
      const response = await axios.get(process.env.REACT_APP_BASE_URL+`/api/results/get-result/${params.resultId}`,values,
        {
          headers: {
  ...
  return (
    <div>
      ...
      {result && result.map((item,index)=>(<>
            ...
            <hr/>
      <h1 className="underline cursor-pointer text-medium"
      onClick={()=>setShowStudentsModal(true)}>Add Student</h1>
        </>    
      ))
      }
      <Modal title='Select Student' open={showStudentsModal} onCancel={()=>setShowStudentsModal(false)}>
